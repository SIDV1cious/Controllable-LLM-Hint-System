...
def save_to_logs(question_id, user_query, ai_response):
    engine = get_db_engine()
    try:
        with engine.connect() as conn:
            sql = text("""
                       INSERT INTO interaction_logs (question_id, student_id, user_query, ai_response,
                                                     is_leaking_answer, created_at)
                       VALUES (:q_id, :s_id, :query, :resp, :leaking, :time)
                       """)
            conn.execute(sql, {
                "q_id": question_id, "s_id": MY_ID, "query": user_query,
                "resp": ai_response, "leaking": 0, "time": datetime.now()
            })
            conn.commit()
...
def generate_report():
    ...
    report += f"- **导出时间**：{datetime.now().strftime('%Y-%m-%d %H:%M')}\n"
...
st.markdown("""
    ...
    [data-testid="stMetricValue"] { text-align: center; color: #00FBFF !important; }
    [data-testid="stMetricLabel"] { text-align: center; width: 100%; color: #E5E7EB !important; font-weight: 600 !important; }
    ...
    .stTextArea textarea { background-color: #1A1C24 !important; color: #FFFFFF !important; border: 1px solid #4B5563 !important; }
    .stTextArea label p { color: #D1D5DB !important; font-weight: 600; }
    ...
    if st.button("🚀 提交并判断对错"):
        if problem_is_locked and student_answer:
            ...
            try:
                ...
                if "正确" in result:
                    st.success("✅ 正确")
                    save_to_logs(1, f"【答案提交】{student_answer}", "正确")
                else:
                    st.error("❌ 错误")
                    st.session_state.trial_count += 1
                    save_to_logs(1, f"【答案提交】{student_answer}", "错误")
                    st.rerun()
            except Exception as e:
                st.error(f"故障：{e}")
    ...
    if problem_is_locked:
        ...
        with st.chat_message("assistant"):
            try:
                ...
                save_to_logs(1, f"【智能辅导】{prompt}", ai_reply)
                st.rerun()
            except Exception as e:
                st.error(f"AI 故障：{e}")
